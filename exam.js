...
function getBank() {
  const g = globalThis;
  const arr = currentLang === 'ar' ? g.LECT_FINAL_EXAM_AR : g.LECT_FINAL_EXAM_EN;
...
function renderGate() {
  ...
  if (online && (!authUser || !authUser.uid)) {
    ...
    ll.href = 'login.html?next=' + encodeURIComponent('exam.html');
...
async function startFresh() {
  ...
  if (isOnline()) {
    const u = onlineUser();
    if (!u || !u.uid) {
      window.location.href = 'login.html?next=' + encodeURIComponent('exam.html');
      return;
    }
  ...
  const total = exam.bank.length;
  const now = Date.now();
  // Effective deadline: personal 45 min, but never past the window close
